...
// ══════════════════════════════════════════════════════════════════
// EL AREA SE PIDE POR BLOQUES, NO POR UNA LISTA DE PARRAFOS
//
// El prompt le pide al modelo seis bloques en cada area: el arranque, HOY,
// ORIGEN, CREENCIAS, SOLTAR y el CIERRE. La escena y el cierre ya tenian
// casilla propia y NUNCA han faltado. Los otros cuatro iban todos revueltos
// en una sola casilla, "parrafos", y ahi dentro no habia forma de saber si
// habia escrito los cuatro o dos.
//
// En el informe del 23 de agosto faltaba ORIGEN —"de donde te viene esto"— en
// CUATRO de las siete areas. Nadie lo comprobaba, y son las 150 palabras por
// area que mas valor le dan. De ahi salen las dos cosas a la vez: el area sale
// corta y le cuenta una sola idea.
//
// Aqui cada bloque pasa a tener su casilla, y cada casilla sus huecos con
// nombre (p1, p2, p3...). Los que van en "required" los rellena SIEMPRE,
// porque la API no le deja terminar la respuesta sin ellos: es el mismo
// mecanismo que ya sostiene la escena y el cierre, el unico de este producto
// que no ha fallado ni una vez.
//
// Y NO, no se puede hacer con minItems: esta API solo acepta minItems 0 y 1, y
// aqui se llama en crudo sin SDK que lo limpie, asi que un minItems 4 no seria
// una regla floja, seria un error 400 y ningun informe. Los huecos con nombre
// y "required" es lo unico que la API garantiza de verdad.
//
// Lo que llega se convierte en la misma lista de parrafos de siempre (ver
// bloquesAParrafos), asi que de ahi para abajo NADA cambia: el montaje, los
// ladillos, las negritas, el nombre y el PDF ven exactamente lo que veian.
// ══════════════════════════════════════════════════════════════════

// Un hueco de parrafo. Las reglas de como se escribe estan en el prompt; aqui
// solo lo justo, porque este trozo viaja en las ocho llamadas de cada informe.
const HUECO_DE_PARRAFO = {
  type: 'object',
  properties: {
    ladillo: { type: ['string', 'null'], description: 'Ladillo de tres a cinco palabras que va ENCIMA de este parrafo, o null si no lleva.' },
    texto: { type: 'string', description: 'Parrafo de 2 a 7 lineas, nunca mas de 90 palabras. Las negritas van aqui, marcadas con **dos asteriscos a cada lado**.' },
  },
  required: ['ladillo', 'texto'],
  additionalProperties: false,
};

// Un bloque son huecos con nombre: los primeros obligatorios, y unos cuantos
// de mas que quedan libres para el area que tenga mas que contar (el AREA 1 va
// entre 1.100 y 1.300 palabras y las otras entre 850 y 900, y el esquema tiene
// que ser identico en las siete llamadas o la cache del prompt no acierta).
function bloqueDeArea(description, obligatorios, deMas) {
  const properties = {};
  const required = [];
  for (let i = 1; i <= obligatorios + deMas; i++) {
    properties['p' + i] = HUECO_DE_PARRAFO;
    if (i <= obligatorios) required.push('p' + i);
  }
  return { type: 'object', description, properties, required, additionalProperties: false };
}

// Las cuatro casillas grandes ya no dicen un numero de parrafo, dicen detras de
// QUE BLOQUE se leen. Dos motivos: el modelo ya no puede saber que numero le va
// a tocar a cada parrafo, y donde va cada una es, segun el propio prompt, "lo
// que mas se falla". Un nombre de una lista de cuatro no se puede fallar.
...
const ESQUEMA_AREA_POR_BLOQUES = {
  type: 'object',
  properties: {
    arranque: bloqueDeArea('EL ARRANQUE del area: abre ancho, desde algo que le pasa a mucha gente, y solo entonces se estrecha hasta ella.', 1, 1),
    hoy: bloqueDeArea('El bloque HOY: como se manifiesta ahora, lo malo Y lo bueno, con el don contado a fondo. Es el bloque mas largo del area.', 4, 2),
    origen: bloqueDeArea('El bloque ORIGEN: por que es asi y de donde le viene, uniendo pasado y presente como causa y efecto. UNA sola explicacion, desarrollada a fondo.', 2, 1),
    creencias: bloqueDeArea('El bloque CREENCIAS: lo que da por cierto sin haberlo puesto en duda y que hace que todo se repita solo. Aqui va la verdad incomoda. Despues de HOY, el que mas sitio ocupa.', 3, 1),
    soltar: bloqueDeArea('El bloque SOLTAR: solo NOMBRA la creencia que tiene que caer. Ni pasos, ni ejercicios, ni plan. Es el bloque mas corto de todos.', 1, 0),
  ...
  required: ['arranque', 'hoy', 'origen', 'creencias', 'soltar', 'escena', 'remate_herida', 'remate_fuerza', 'pregunta', 'cierre'],
  additionalProperties: false,
};
...
// Convierte los bloques en la lista de parrafos que espera todo lo de abajo, y
// traduce el "tras_bloque" de las casillas grandes al numero de parrafo de
...
function bloquesAParrafos(datos) {
  ...
  for (const nombre of ORDEN_DE_LOS_BLOQUES) {
    const bloque = datos && datos[nombre];
    let puestos = 0;
    if (bloque && typeof bloque === 'object') {
      // Los huecos se leen por su numero, no por el orden en que el modelo los
      // haya escrito en el JSON: p2 va detras de p1 aunque llegue antes.
      const huecos = Object.keys(bloque)
        .filter(k => /^p\d+$/.test(k))
        .sort((a, b) => Number(a.slice(1)) - Number(b.slice(1)));
      for (const hueco of huecos) {
        const p = bloque[hueco];
    ...
    // queda donde acabo el anterior, asi lo que fuera detras de el no se
    // descoloca.
...
- REGLA CRÍTICA DE LONGITUD: cada área tiene OBLIGATORIAMENTE entre 850 y 900 palabras, con UNA excepción: el ÁREA 1 (IDENTIDAD) va entre 1.100 y 1.300 palabras, porque cubre más terreno. Los párrafos salen de los huecos que pide cada bloque: los obligatorios van todos, y los de más se usan cuando de verdad tienes más que contarle. Un área por debajo de su mínimo es un ERROR GRAVE que rompe el producto final. Si te sale corta, AMPLÍA con más detalle y más ejemplos, AÑADIENDO párrafos nuevos, nunca engordando los que ya tienes.
...
- El texto del área va en CINCO CASILLAS, una por bloque: arranque, hoy, origen, creencias y soltar. Cada casilla tiene sus huecos numerados (p1, p2, p3...) y cada hueco es un párrafo. Los huecos que la casilla pide van SIEMPRE escritos, sin excepción; los de más solo se usan si de verdad tienes más que contarle de ella. No colocas los bloques ni los ordenas: se leen siempre en ese orden y de eso se encarga el código.
...
No pongas título de área ni encabezado: el título ya va impreso en la página. Rellena todas las casillas: los cinco bloques de texto con sus huecos y sus ladillos, la escena, los dos remates, la pregunta y el cierre. Entre 1.100 y 1.300 palabras, en párrafos de longitud variada, entre 2 y 7 líneas, ninguno de más de 90 palabras.`
    },
    {
      id: 2,
...
8. Ningun hueco obligatorio de los cinco bloques se queda sin escribir, ni uno. Y si el area se te queda corta para las palabras que te piden, tira de los huecos de mas que lleva cada bloque: estan para eso
...
COPIALAS TAL CUAL, letra por letra, tal como estan escritas en el texto, para que se puedan encontrar. No las reescribas ni las arregles.`,
  ...
  async function pedirArea(area, loQueFalloAntes) {
    ...
    // El area llega por bloques: aqui se convierten en la lista de parrafos de
