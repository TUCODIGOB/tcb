...
async function guardarContactoBrevo(datos) {
  ...
  // 15 minutos, a la hora y a las tres horas. Ninguna compra se queda sin su
  // ficha porque Brevo estuviera caido justo en ese segundo.
  try {
    await guardarContactoEnBrevo(body, BREVO_API_KEY, 8000);
  } catch (err) {
    await apuntarContactoDeBrevo(body, err.message);
    throw err;
  }
