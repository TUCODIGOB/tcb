...
async function guardarContactoBrevo(datos) {
  ...
  // EL TELEFONO NO PUEDE COSTARLE LA FICHA A UN CLIENTE QUE YA HA PAGADO.
  // Si ese movil ya esta en otro contacto, Brevo rechaza este ENTERO: ni la
  // lista de compradores, ni sus datos de nacimiento, ni nada. Asi que se
  // guarda otra vez sin el telefono, que es un dato de mas: el numero ya esta
  // en el otro contacto, y lo que importa es que su compra quede registrada.
  if (!resp.ok && attributes.SMS) {
    const queja = await resp.text();
    if (!esElTelefonoRepetido(resp.status, queja)) {
      throw new Error(`Brevo ${resp.status}: ${queja}`);
    }
  ...
  // Ya ha comprado, asi que deja de ser un carrito abandonado y deja de ser
  // un lead del regalo: se le saca de esas dos listas para que no se quede en
  // varias a la vez. Va aparte porque el alta del contacto no permite
  // desvincular listas, solo vincularlas.
  ...
  try {
    const quitar = await fetch(`https://api.brevo.com/v3/contacts/${encodeURIComponent(datos.email)}`, {
      ...
      // 11 = "1 P1-carrito-abandonado"; 14 = "0 P1 Lead Magnet".
      body: JSON.stringify({ unlinkListIds: [11, 14] }),
    });
    if (!quitar.ok) {
      console.error(`No se pudo sacar de sus listas anteriores (${quitar.status}):`, await quitar.text());
    }
  } catch (err) {
