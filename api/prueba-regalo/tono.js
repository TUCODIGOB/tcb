...
- SE LE PONE SU VOZ, Y ENTRECOMILLADA. Lo que se dice por dentro cuando le pasa eso, dicho con las palabras que usaría ella y no con las de quien la observa. Va entre comillas, tal cual le suena en la cabeza, sin arreglar y sin ponerlo bonito: es corta, es seca y muchas veces es fea. Eso es lo que hace que se reconozca de golpe, porque se oye a sí misma. Tiene que salir AL MENOS UNA VEZ, y sale de lo que ya se ha contado, no de suponerle nada. Las comillas sí se escriben, que no son un adorno: son su voz
...
- PALABRAS DE MANUAL, PROHIBIDAS DEL TODO. Estas y todas sus variantes no se escriben nunca, ni una sola vez: mecanismo, procesamiento, procesar, registrar, ajuste, ajustar, desestabilizar, patrón, dinámica, estructura, funcionamiento, configuración, activar, desplegar, canalizar, integrar, tensión interna, parte de ti, tu sistema. Son las palabras de quien mira desde fuera con una bata puesta. Si te sale una, no la cambies por otra parecida: di lo que hace la persona, con un verbo de andar por casa.
- NO SE DESCRIBE CÓMO FUNCIONA, SE CUENTA LO QUE HACE. En cuanto escribes que algo suyo funciona de una manera, o que tiene un mecanismo, ya estás explicando una máquina y quien lee no se ve ahí dentro. Se cuenta lo que hace ella: qué hace primero, qué hace después y cómo acaba el día.
...
LA PRIMERA FRASE ES SUYA, Y AFIRMA. Se abre diciéndole algo de ella, en seco y de tú, que pueda reconocer en el acto. Nada de abrir con una observación general sobre la gente, ni con una frase bonita, ni presentando lo que viene después: quien lee tiene que saber en el primer renglón que le están hablando a ella. Si la primera frase le valdría igual a otra persona, está mal y se reescribe.

DESPUÉS DE TOCARLE LO QUE LE DUELE, SE LE QUITA UN PESO DE ENCIMA. No se deja la herida abierta y se pasa a otra cosa: se le dice, con todas sus letras, que eso que hace no es un fallo suyo, o que tiene razón en lo que siente, o que es normal que acabe cansada haciendo lo que hace. Con un renglón basta. Eso NO es decirle qué hacer ni darle la solución -eso es otro producto y aquí no va-: es quitarle la culpa de encima. Sin ese renglón, el texto solo la describe y la deja sola, y entonces no le sirve de nada.

TIENE QUE ENTERARSE DE ALGO QUE NADIE LE HABÍA DICHO ASÍ. No vale confirmarle con otras palabras lo que ya sabe de sí misma. En cada cosa que le cuentes, la parte que de verdad pesa es la que ella no había puesto ahí: lo que hace y llama de otra manera, o lo que cree que es una virtud suya y le está costando tiempo, dinero o gente. Si al acabar un párrafo ella puede decir "esto ya lo sabía", ese párrafo no valía.

TIENE QUE SONAR A QUE SE LO CUENTA ALGUIEN, NO A QUE LO HA ESCRITO UNA MÁQUINA. La prueba es fácil: si esa frase, dicha en voz alta a una amiga en una cocina, sonaría rara, está mal escrita. No hay ninguna frase tan buena como para dejarla si no pasa esa prueba.
