// ═════════════════════════════════════════════════════════════════
// /api/prueba-regalo/carta.js
// Calcula la carta natal para la pagina de prueba del regalo.
//
// ES EL MISMO CALCULO QUE EL DEL P1, COPIADO TAL CUAL.
//
// No se toca api/calcular-carta.js ni nada del P1: aquel exige haber pagado
// y esta prueba no paga, asi que vive aparte y no puede romper nada de alli.
// El bloque astronomico de mas abajo esta copiado linea por linea, sin cambiar
// ni una formula.
//
...
// ─── EL LUGAR: DE LO QUE ESCRIBE A COORDENADAS ───────────────────────────────
//
// Se le pregunta al mismo mapa que usa el P1 y se coge el primer resultado.
// Aqui no hace falta la maquinaria del P1 para decidir CUAL de los tres campos
// esta mal escrito: eso es para marcarselos en rojo a una clienta, y esto es
// una pagina de trabajo.
async function buscarElLugar(municipio, provincia, pais) {
  const consulta = [municipio, provincia, pais].filter(Boolean).join(', ');
  ...
  const INTENTOS = 3; // el primero y dos reintentos, igual que en el P1
  for (let intento = 1; intento <= INTENTOS; intento++) {
    try {
      const r = await fetch(url, { headers: { 'User-Agent': 'TuDisenoDeOrigen/1.0' } });
      if (!r.ok) throw new Error('el mapa ha contestado ' + r.status);
...
// ─── LA ZONA HORARIA ─────────────────────────────────────────────────────────
//
// Copiado de api/timezone.js, que es lo que usa el P1: da el desfase que REGIA
// ese dia en ese sitio, con los horarios de verano historicos de cada pais.
...
export default async function handler(req, res) {
  ...
  try {
    const lugar = await buscarElLugar(String(municipio).trim(), String(provincia).trim(), String(pais).trim());
    if (!lugar) {
      return res.status(404).json({ error: 'No hemos encontrado ese lugar de nacimiento' });
    }
    ...
    return res.status(200).json({
      carta,
      lugar: { lat: lugar.lat, lon: lugar.lon, nombre: lugar.nombre },
      zona,
      tzOffset,
    });
