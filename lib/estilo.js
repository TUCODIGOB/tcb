...
export function vecesQueSaleElNombre(texto, nombre) {
  const n = normalizar(nombre).trim();
  if (!n) return 0;
  const t = normalizar(texto);
  // Se busca como palabra suelta para que "Ana" no cuente dentro de "manana".
  const re = new RegExp('(^|[^a-z0-9])' + n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?![a-z0-9])', 'g');
