...
// LA COMA QUE CIERRA UN VOCATIVO NO SE TOCA.
//
// "Tu eres de las segundas, Raquel, y llevas tanto tiempo..." salio impreso
// como "...segundas, Raquel y llevas...", sin la coma de cierre, porque
// detras de la "y" venia "llevas" y la regla la dio por sobrante. Pero esa
// coma no separa dos frases: cierra el nombre. Si delante de la coma esta el
// nombre de la persona, se deja como esta.
export function quitarComaAntesDeY(texto, nombre) {
  const suNombre = String(nombre || '').trim();
  const cierraElNombre = suNombre
    ? new RegExp('(^|[^a-zA-ZáéíóúñÁÉÍÓÚÑ])' + suNombre.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$', 'i')
    : null;
  return String(texto || '').replace(
    /,(\s+[ye]\s+)([^\s,.;:]+(?:\s+[^\s,.;:]+)?)/gi,
    (todo, medio, sigue, donde, entero) => {
      if (cierraElNombre && cierraElNombre.test(entero.slice(0, donde))) return todo;
