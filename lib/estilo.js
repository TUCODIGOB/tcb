...
// cualquier "cuando sale el sol" del texto colaba como si la hubiera
// nombrado, y entonces nadie reclamaba que faltara: esa clienta se quedaba
// sin que la llamaran por su nombre en las 26 paginas y sin que saltara nada.
//
// Lo que se cuenta es el VOCATIVO, que es la forma de llamar a alguien y en
// castellano va siempre entre comas: "y ahi esta, Raquel, lo que no ves".
// Asi funciona igual para todos los nombres, y de paso lo que se cuenta es
// exactamente lo que se pide, no una palabra suelta que pasaba por ahi.
export function vecesQueLaLlamaPorSuNombre(texto, nombre) {
  const n = normalizar(nombre).trim();
  if (!n) return 0;
  const t = normalizar(texto);
  const escapado = n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // Delante: principio de texto o un signo que abre. Detras: un signo que
  // cierra. Entre medias, el nombre y nada mas.
