...
// La ficha de un email: sus datos de nacimiento, su carta, y lo que el regalo
// le dejo escrito.
//
...
//
// Y SE PUEDE LEER. Lo que el regalo dejo escrito -su area 1 y los rasgos con
// los que se escribio- esta ahi dentro, y el informe lo necesita para
// continuar por donde ella lo dejo en vez de empezar de cero.
//
// NUNCA CORTA NADA. Quien llama no espera a que esto termine: sale a la vez
// que se escribe el informe, y si falla se deja aviso y el informe sigue su
// camino. Guardar no puede retrasar ni estropear lo que ha pagado.
//
...
// LA PETICION FIRMADA. La misma para mirar si esta -HEAD, que no baja nada- y
// para leerla entera -GET-.
async function pedirLaFicha(metodo, huella) {
  const cfg = ajustes();
...
// ¿ESTA YA? Solo la cabecera: lo unico que hace falta saber es si existe.
async function yaLaTiene(huella) {
  const resp = await pedirLaFicha('HEAD', huella);
  if (resp.status === 404) return false;
  if (!resp.ok) throw new Error(`R2 ${resp.status}`);
  return true;
}

// LEERLA. Devuelve lo que hay guardado de ese email, o null si no tiene nada.
// Un email sin ficha no es un fallo: es alguien que no ha pasado por el
// regalo, y quien llama sigue su camino.
export async function leerLaFicha(email) {
  const huella = huellaDelEmail(email);
  if (!huella) return null;

  const resp = await pedirLaFicha('GET', huella);
