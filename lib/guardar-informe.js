...
//
// QUE SE GUARDA: solo texto. Los rasgos, las siete areas y sus datos, la carta
// natal con la que se hicieron, y el cuaderno de como fue -tiempos, gasto y
// limpieza- con el numero de intento en el que salio. Unas pocas decenas de KB por informe.
// Ni el PDF, ni las imagenes, ni las fuentes: el PDF se vuelve a montar desde
// el texto, y pesa 1,8 MB.
//
// LA CARTA SE GUARDA AUNQUE PAREZCA QUE SE PUEDE RECALCULAR. Si, sale de la
// fecha, la hora y el lugar; pero las coordenadas exactas del lugar las busca
// el navegador de la clienta en el momento de la compra y no quedan escritas
// en ninguna otra parte. Volver a buscarlas dentro de un ano puede devolver un
// punto algo distinto, y con el una carta que ya no es la que ella leyo. Lo
// que se guarda aqui son los grados tal y como se calcularon ese dia: los
// planetas, las casas, los aspectos, la latitud y la longitud. Son un par de
// KB al lado de las decenas que ya ocupa el texto, asi que no cambia nada.
//
...
// LEERLO DE VUELTA. Devuelve lo que se guardo de esa compra, o null si no hay
// nada. Se usa cuando el informe ya esta escrito y lo unico que falta es
// entregarlo: con esto se vuelve a montar el PDF sin escribir nada de nuevo.
export async function leerInforme({ producto, sessionId }) {
  const cfg = ajustes();
  ...
  const camino = `/${cfg.bucket}/${cual}/${compra}.json`;
  ...
  const peticion = ['GET', camino, '', cabeceras, firmadas, hash].join('\n');
...
export async function guardarInforme({ producto, sessionId, cliente, carta, areas, rasgos, cuaderno, intento }) {
  const cfg = ajustes();
  if (!cfg) {
    console.warn('Informe no guardado: faltan las variables INFORME_P1_CLOUDFLARE_*');
  ...
  const cuerpo = Buffer.from(JSON.stringify({
    ...
    areas,
    rasgos,
    // EL CUADERNO Y EL INTENTO son solo para mirar: los tiempos de cada
    // llamada, lo que costo, lo que quito la limpieza, y si salio a la primera
    // o en que reintento. No entran en el informe ni cambian nada de lo que se
    // entrega; si no vinieran, el fichero se guarda igual.
    cuaderno: cuaderno || null,
    intento: intento || null,
  }), 'utf8');

  const ruta = `${cual}/${compra}.json`;
  // La direccion del bucket, tal cual la da Cloudflare en Settings > S3 API.
  // El de las resenas lleva ".eu." porque se creo con jurisdiccion europea;
  // este no la tiene, y usar la europea con un bucket que no lo es devuelve
  // "AccessDenied" -que despista, porque suena a permisos y no lo es-.
  const host = `${cfg.cuenta}.r2.cloudflarestorage.com`;
  const region = 'auto', servicio = 's3';
  const ahora = new Date();
  const dia = ahora.toISOString().slice(0, 10).replace(/-/g, '');
