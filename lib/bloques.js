...
export function revisarBloques(bloques, opciones) {
  ...
  // Las negritas. Al que escribe no se le da ningun numero: se le pide que
  // marque como subrayaria un lector con un rotulador, todo lo que le sacude.
  // Aqui abajo hay un suelo, que es otra cosa: no es el objetivo, es lo que
  // marcaria una persona leyendo, uno cada dos parrafos mas o menos. En un
  // area de 900 palabras salen cinco; en la primera, que ronda las 1.250,
  // salen siete. Por encima de eso no se pide nada: si el area trae diez, diez
  // se quedan, que de eso se trata.
  // Se cuentan solo las del texto corrido, porque la escena, los remates, las
  // preguntas y el cierre no llevan negrita por diseño.
  let negritas = 0, palabras = 0;
  for (const b of bloques) {
    if (b.tipo !== 'texto' && b.tipo !== 'cierre') continue;
    palabras += b.t.split(/\s+/).length;
    if (b.tipo !== 'texto') continue;
    negritas += Math.floor(((b.t.match(/\*\*/g) || []).length) / 2);
  }
  const minNegritas = Math.min(7, Math.max(5, Math.round(palabras / 170)));
  if (negritas < minNegritas) {
    fallos.push(`solo hay ${negritas} frases en negrita y hacen falta ${minNegritas} como mínimo: va en negrita todo lo que sacude, y en un área de ${palabras} palabras eso siempre es más de lo que hay ahora`);
  }
