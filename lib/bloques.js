...
export function montarArea(datos) {
  ...
  //
  // Detras del ultimo parrafo solo va el cierre, que tambien se imprime grande
  // y centrado. Una frase grande ahi deja dos carteles seguidos sin una linea
  // de texto en medio, y el cierre, que es el golpe del area, pierde el efecto
  // porque llega detras de otro igual. Antes esto se pedia en el prompt y no se
  // comprobaba, asi que pasaba: el tope se queda un parrafo mas arriba y asi
  // detras de cada frase grande queda siempre texto.
  //
  ...
  // puede cumplir, y forzarlo dejaria la frase fuera del area.
  //
  // El tope vale para las frases grandes, que son las que compiten con el
  // cierre. La escena no: va en cursiva gris y con su filete, asi que pegada al
  // cierre no hace el cartel doble. Por eso puede usar el ultimo parrafo cuando
  // el area viene tan corta que no quedan huecos para las cuatro.
  const ultimoConTextoDetras = Math.max(1, parrafos.length - 1);
  const topeDe = marca => (marca === 'ESCENA' ? parrafos.length : ultimoConTextoDetras);
  ...
  const pedidos = [
    ...
    .map(([marca, d]) => {
      ...
      return { marca, texto: String(d.texto).trim(), sitio: Math.min(Math.max(Math.round(n), 1), topeDe(marca)) };
    })
  ...
  for (const item of pedidos) {
    let n = item.sitio;
    const tope = topeDe(item.marca);
    while (ocupados.has(n) && n < tope) n++;
    ...
    // resigna a compartir parrafo, y eso ya es un area que no da de si.
    if (ocupados.has(n)) {
      let m = item.sitio;
      while (m > 1 && ocupados.has(m)) m--;
      if (!ocupados.has(m)) n = m;
      // Sin un solo hueco libre con texto detras, se usa el ultimo parrafo
      // aunque quede contra el cierre: perder una frase grande al sanearla es
      // peor. Hacen falta cinco parrafos para las cuatro casillas, y un area de
      // verdad trae once o mas, asi que esto solo salta con datos de prueba.
      else if (!ocupados.has(parrafos.length)) n = parrafos.length;
    }
    ocupados.add(n);
