...
export function montarArea(datos) {
  ...
  // parrafo, que el area abre con texto, y nunca detras del ULTIMO.
  //
  ...
  //
  // Y DETRAS DEL ULTIMO PARRAFO SOLO VA EL CIERRE, que tambien se imprime
  // grande y centrado: una casilla ahi deja dos carteles seguidos sin una
  // linea de texto en medio, y el golpe del area se pierde porque llega detras
  // de otro igual. Vale para las cuatro, la escena incluida, que un ejemplo al
  // final tambien corta el area justo antes del golpe.
  //
  // Se pedia en el prompt y no se comprobaba, asi que pasaba: en las areas 5, 6
  // y 7 del ultimo informe la pregunta salio pegada al cierre. Llega
  // ahi cuando el cierre viene de relleno y chat.js lo cambia por el ultimo
  // parrafo del area: las casillas ya tenian su sitio puesto, y la que
  // apuntaba al penultimo se queda la ultima. Por eso el tope se aplica aqui,
  // al final y con los parrafos ya definitivos, y no cuando se reparten.
  //
  // Con un solo parrafo no hay donde ponerla que no sea el ultimo: ahi no se
  // puede cumplir, y forzarlo dejaria la casilla fuera del area.
  const ultimoConTextoDetras = Math.max(1, parrafos.length - 1);
  ...
  const pedidos = [
    ...
    .map(([marca, d]) => {
      ...
      return { marca, texto: String(d.texto).trim(), sitio: Math.min(Math.max(Math.round(n), 1), ultimoConTextoDetras) };
    })
  ...
  // UN PARRAFO QUE NO ACABA LA FRASE NO ES SITIO PARA UNA CASILLA GRANDE.
  //
  // Estas cuatro se imprimen grandes y centradas, y van DETRAS del parrafo que
  // les toca. Si ese parrafo se queda a mitad de frase, la casilla se mete por
  // el medio y parte la frase en dos.
  //
  // En el informe del 30, area 2, salio impreso asi:
  //
  //   "...acaba saliendo a medias, envuelto en un
  //    ¿Cuantas veces has dado tres vueltas a algo que podias haber pedido en
  //    una frase?
  //    no importa, tampoco es para tanto"."
  //
  // El modelo habia partido "envuelto en un "no importa, tampoco es para
  // tanto"" en dos parrafos, y la pregunta cayo en el hueco. El codigo hizo
  // lo que le tocaba -ponerla detras del parrafo 3- y el resultado no se puede
  // leer.
  //
  // Asi que un parrafo solo vale como sitio si termina una frase. Se miran las
  // comillas y los parentesis del final antes de mirar el punto, que una frase
  // acabada puede cerrar con ellos.
  const cierraFrase = (n) => {
    const p = parrafos[n - 1];
    // Se quitan del final las comillas, los parentesis y los asteriscos de la
    // negrita antes de buscar el punto: un parrafo que remata con una negrita
    // acaba en "**" y el punto se queda dentro, asi que mirando el ultimo
    // caracter a secas parecerian todos frases a medias.
    const t = String((p && p.texto) || '').trim().replace(/[*"'»)\]]+$/, '').trim();
    return /[.?!\u2026]$/.test(t);
  };
  ...
  // PERO ACABAR LA FRASE ES UNA PREFERENCIA, Y NO AMONTONARSE ES UNA REGLA.
  //
  // Las dos condiciones no valen lo mismo, y pedirlas a la vez costo un
  // informe. El 27 de agosto se pidieron las dos juntas: un area cuyos
  // parrafos iban encadenados -acabados en coma, que es como se escribe
  // cuando una idea sigue en el parrafo siguiente- no tenia NI UN sitio
  // valido, asi que las cuatro casillas se amontonaron al final, pegadas una
  // detras de otra. Y dos casillas pegadas sin texto en medio se leen como un
  // cartel: al sanearlas una deja de ser frase grande y se pierde. El area
  // llegaba sin remates, se rechazaba y se pedia entera otra vez. Tres
  // minutos y el doble de gasto por informe.
  //
  // Partir una frase se lee raro. Perder un remate deja el area coja y la
  // manda a rehacer. Asi que se busca en dos vueltas: primero un sitio libre
  // QUE ADEMAS cierre frase, y si en toda el area no hay ninguno, vale
  // cualquier sitio libre, que es como se hacia antes de todo esto.
  // Compartir parrafo sigue siendo el ultimo recurso, y solo cuando no queda
  // ni un hueco libre.
  //
  // Se busca hacia abajo desde donde la pidio el modelo y, si por ahi no
  // queda nada, hacia arriba. Lo que no hace en ninguna de las dos vueltas es
  // usar el ultimo parrafo, que ahi el cierre se quedaria sin su linea de
  // texto delante.
  const buscarSitio = (sitio, pidiendoFrase) => {
    const vale = n => !ocupados.has(n) && (!pidiendoFrase || cierraFrase(n));
    let n = sitio;
    while (!vale(n) && n < ultimoConTextoDetras) n++;
    if (vale(n)) return n;
    let m = sitio;
    while (m > 1 && !vale(m)) m--;
    return vale(m) ? m : null;
  };

  for (const item of pedidos) {
    // 1) libre y con la frase acabada. 2) libre, aunque parta la frase.
    let n = buscarSitio(item.sitio, true);
    if (n === null) n = buscarSitio(item.sitio, false);
    // 3) Ni uno libre en toda el area, y eso ya es un area que no da de si:
    // hacen falta cinco parrafos para las cuatro casillas y una de verdad
    // trae once o mas. Se resigna a compartir, igual que siempre.
    if (n === null) {
      n = item.sitio;
      while (ocupados.has(n) && n < ultimoConTextoDetras) n++;
    }
    ocupados.add(n);
  ...
  // Del cierre se imprime SU TEXTO y nada mas: la casilla "revela" que va a
  // su lado es la nota con la que el modelo se obliga a encontrar la
  // revelacion antes de escribirla, y no es para la clienta. Ver chat.js.
  const cierre = typeof datos?.cierre?.texto === 'string' ? datos.cierre.texto.trim() : '';
...
export function revisarBloques(bloques, opciones) {
  ...
  // Las tres opcionales las pone quien ya ha decidido a proposito entregar el
  // area sin esa pieza, porque la que venia era basura y no se pudo arreglar.
  // Sin ese aviso explicito, las tres siguen siendo obligatorias.
  const opcional = que => !!(opciones && opciones[que]);
  if (!opcional('escenaOpcional') && cuantos('escena') < 1) {
    fallos.push('el área ha llegado sin escena');
  }
  if (!opcional('remateOpcional') && cuantos('remate') < 1) {
    fallos.push('el área ha llegado sin ningún remate: son dos, el de la herida y el de la fuerza');
  }
  if (!opcional('preguntaOpcional') && cuantos('pregunta') < 1) {
    fallos.push('el área ha llegado sin pregunta: sin ninguna, se lee de corrido y sin una sola pausa');
  }
  const ultimo = bloques[bloques.length - 1];
  if (!ultimo || ultimo.tipo !== 'cierre') {
    fallos.push('el área no termina con el párrafo de cierre');
  }
  return fallos;
}
