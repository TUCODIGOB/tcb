...
export function montarArea(datos) {
  ...
  const colocar = (sitio, marca, texto) => {
    if (!texto || !String(texto).trim()) return;
    let n = Number(sitio);
    if (!Number.isFinite(n)) n = 1;
    n = Math.min(Math.max(Math.round(n), 1), parrafos.length);
    if (!dentro.has(n)) dentro.set(n, []);
    dentro.get(n).push(`[${marca}] ${String(texto).trim()}`);
  };
  colocar(datos?.escena?.tras_parrafo, 'ESCENA', datos?.escena?.texto);
  colocar(datos?.remate_herida?.tras_parrafo, 'REMATE', datos?.remate_herida?.texto);
  colocar(datos?.remate_fuerza?.tras_parrafo, 'REMATE', datos?.remate_fuerza?.texto);
  colocar(datos?.pregunta?.tras_parrafo, 'PREGUNTA', datos?.pregunta?.texto);
