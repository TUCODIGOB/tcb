...
export function revisarBloques(bloques, opciones) {
  ...
  // subraye como subraya un lector con un rotulador, todo lo que le sacude.
  // Aqui abajo hay un suelo, que es otra cosa: no es el objetivo, es el aviso
  // de que un area con una sola frase subrayada en novecientas palabras no se
  // ha subrayado, y en el informe que se genero de verdad pasaba eso. Se
  // cuentan solo las del texto corrido, porque la escena, los remates, las
  ...
  if (negritas < 3) {
    fallos.push(`solo hay ${negritas} frases subrayadas en negrita y el área se lee sin nada que destaque: hay que subrayar todo lo que sacude, que en un área de estas siempre es más de una cosa`);
  }
