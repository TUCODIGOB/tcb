...
export function analizarArea(texto) {
  ...
  for (let i = 0; i < parrafos.length; i++) {
    let cuerpo = parrafos[i].trim();
    if (!cuerpo) continue;

    let tipo = 'texto';
    let marcado = false;
    const marca = CON_CORCHETES.exec(cuerpo) || SIN_CORCHETES.exec(cuerpo);
    if (marca) {
      tipo = tipoDe(marca[1]);
      marcado = true;
      cuerpo = cuerpo.slice(marca[0].length).trim();
    }

    cuerpo = cuerpo.replace(MARCA_SUELTA, ' ').replace(/[ \t]{2,}/g, ' ').trim();
    if (!cuerpo) continue;

    if (tipo === 'texto' && esPreguntaSuelta(cuerpo)) tipo = 'pregunta';
    bloques.push({ tipo, t: cuerpo, marcado });
  }
...
// Lo que tiene que traer un area para poder maquetarse. Si falta algo, el
// area no se entrega: se vuelve a pedir. Un area sin marcas se lee como el
// muro de texto de siempre, y eso es justo lo que se esta arreglando.
export function revisarBloques(bloques, opciones) {
  const minSub = (opciones && opciones.minSub) || 3;
  const fallos = [];
  ...
  if (subs.length > minSub + 3) {
    fallos.push(`hay ${subs.length} subtítulos y son demasiados, con ${minSub} o cuatro basta`);
  }
  for (let i = 0; i < subs.length; i++) {
    if (subs[i].t.length > 60) {
  ...
  if (remates.length < 2) {
    fallos.push(`hay ${remates.length} remates y son dos, el de la herida y el de la fuerza, cada uno solo en su párrafo empezando por [REMATE]`);
  }
  if (remates.length > 2) {
    fallos.push(`hay ${remates.length} remates y son exactamente dos`);
  }
