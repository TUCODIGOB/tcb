...
export function montarArea(datos) {
  ...
  // acabada puede cerrar con ellos. Si no hubiera ni uno que cierre frase -que
  // no pasa en un area de verdad-, se queda donde habria caido y se comporta
  // como hasta ahora.
  const cierraFrase = (n) => {
  ...
  for (const item of pedidos) {
    let n = item.sitio;
    while ((ocupados.has(n) || !cierraFrase(n)) && n < ultimoConTextoDetras) n++;
    // Si ya no quedan huecos por debajo, se busca hacia arriba, saltando
    // tambien los ocupados. Solo si no queda ni uno libre en toda el area se
    // resigna a compartir parrafo, y eso ya es un area que no da de si: hacen
    // falta cinco parrafos para las cuatro casillas y una de verdad trae once o
    // mas. Lo que no hace ni asi es usar el ultimo, que ahi el cierre se
    // quedaria sin su linea de texto delante.
    if (ocupados.has(n) || !cierraFrase(n)) {
      let m = item.sitio;
      while (m > 1 && (ocupados.has(m) || !cierraFrase(m))) m--;
      if (!ocupados.has(m) && cierraFrase(m)) n = m;
    }
    ocupados.add(n);
