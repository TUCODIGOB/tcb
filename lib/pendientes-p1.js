...
// ── LA MARCA EN BREVO ──────────────────────────────────────────
//
// Deja escrito en su ficha de Brevo como va lo suyo: si esta pendiente, si ya
// se le entrego o si se quedo sin salir, y cuantas veces se ha intentado. Es
// solo para mirarlo desde alli, no decide nada.
//
// updateEnabled deja actualizar a quien ya esta. Sin listIds a proposito: no
// se mete a nadie en ninguna lista desde aqui. Si Brevo no contesta, se apunta
// en el registro y se sigue.
export async function marcarEnBrevo({ email, estado, intentos }) {
  const BREVO_API_KEY = process.env.BREVO_API_KEY;
  if (!BREVO_API_KEY) {
    console.error('[p1] Sin BREVO_API_KEY: no se marca el contacto');
    return false;
  }
  ...
  try {
    const resp = await fetch('https://api.brevo.com/v3/contacts', {
      ...
      body: JSON.stringify({
        email,
        attributes: { P1_ESTADO: estado, P1_INTENTOS: Number(intentos || 0) },
        updateEnabled: true,
      }),
    });
    if (!resp.ok) {
      console.error(`[p1] Brevo ${resp.status} al marcar el contacto: ${(await resp.text()).slice(0, 300)}`);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[p1] No se ha podido marcar el contacto:', err.message);
    return false;
  }
}
...
export async function apuntarPendiente({ sessionId, email, nombre }) {
  const suEmail = String(email || '').trim().toLowerCase();
  const hecho = await guardarPendiente({
  ...
  // Y queda marcado como pendiente en su ficha de Brevo. Va detras del
  // apunte, que es lo que de verdad importa, y no puede cortarlo: la marca es
  // para mirar.
  await marcarEnBrevo({ email: suEmail, estado: 'pendiente', intentos: 0 });
